  ...
  try {
    ...
    const response = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'line_items[0][price_data][product_data][name]': `La Luna Gift Card - $${amount}`,
        'line_items[0][price_data][product_data][description]': `Gift card for ${recipientName}`,
        'line_items[0][price_data][unit_amount]': String(amount * 100),
        'line_items[0][quantity]': '1',
        'mode': 'payment',
        'success_url': `${origin}/laluna/gift/success?session_id={CHECKOUT_SESSION_ID}`,
        'cancel_url': `${origin}/laluna/gift`,
        ...
        'metadata[type]': 'laluna_gift_card',
      }).toString(),
    });
